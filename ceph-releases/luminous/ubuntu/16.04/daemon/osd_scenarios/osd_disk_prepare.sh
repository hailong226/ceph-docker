...
function osd_disk_prepare {
  ...
  if [[ ${OSD_BLUESTORE} -eq 1 ]]; then
    ...
    if [[ -n "${OSD_JOURNAL}" ]]; then
      ceph-disk -v prepare "${CLI_OPTS[@]}" --journal-uuid "${OSD_JOURNAL_UUID}" --lockbox-uuid "${OSD_LOCKBOX_UUID}" "${OSD_DEVICE}" "${OSD_JOURNAL}"
    else
      ceph-disk -v prepare "${CLI_OPTS[@]}" --journal-uuid "${OSD_JOURNAL_UUID}" --lockbox-uuid "${OSD_LOCKBOX_UUID}" "${OSD_DEVICE}"
    fi
